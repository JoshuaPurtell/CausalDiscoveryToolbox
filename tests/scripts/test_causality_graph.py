...
data_graph = pd.read_csv(
    "{}/../datasets/Example_graph_numdata.csv".format(os.path.dirname(os.path.realpath(__file__)))
).iloc[:50, :5]
...
def test_graph():
    for method in [GS, GIES, IAMB, Fast_IAMB, CAM, CCDr, PC, LiNGAM, CCDr, GES, MMPC, Inter_IAMB]:
        print(method)
        m = method()
...
def test_directed():
    for method in [GS, GIES, IAMB, Fast_IAMB, PC, GES, MMPC, Inter_IAMB]:
        print(method)
        m = method()
        output1 = m.predict(data_graph)
        print(nx.adjacency_matrix(output1).todense())
